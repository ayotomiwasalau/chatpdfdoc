...
class UploadService:
    ...
    async def upload_svc(self, file: UploadFile):
        if not file or not file.filename or not file.filename.strip():
            self.log_data.add_log(f"Upload failed - file name must not be empty")
            raise HTTPException(status_code=422, detail="File name must not be empty")

        if not file.content_type.startswith('application/pdf'):
            self.log_data.add_log(f"Upload failed - only PDF files are supported")
